...
class Api::CartsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "can list cart product" do
    ...
    cart_product = response.parsed_body["cart"].as_json["cartProducts"]
  ...
  test "can search cart_product with description" do
    ...
    assert_equal("Cage verte adaptée pour les petits oiseaux.", cart_product.as_json['cart'].as_json['cartProducts'].first.as_json['products'].first.as_json['description'].to_s)
  end
